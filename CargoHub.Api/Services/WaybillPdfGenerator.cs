...
public class WaybillPdfGenerator
{
    ...
    public byte[] Generate(BookingDetailDto booking) =>
        GenerateCombined(new[] { booking });

    /// <summary>One page per booking, single PDF document.</summary>
    public byte[] GenerateCombined(IReadOnlyList<BookingDetailDto> bookings)
    {
        if (bookings == null || bookings.Count == 0)
            throw new ArgumentException("At least one booking is required.", nameof(bookings));
        ...
        var doc = Document.Create(container =>
        {
            foreach (var booking in bookings)
                container.Page(page => ComposeWaybillPage(page, booking));
        });

        return doc.GeneratePdf();
    }

    private void ComposeWaybillPage(PageDescriptor page, BookingDetailDto booking)
    {

...
public sealed class UpdateDraftCommandHandler : IRequestHandler<UpdateDraftCommand, BookingDetailDto?>
{
    ...
    public async Task<BookingDetailDto?> Handle(UpdateDraftCommand request, CancellationToken cancellationToken)
    {
        ...
        if (r.Shipper != null) draft.Shipper = CreateBookingCommandHandler.MapParty(r.Shipper) ?? new HiavaNet.Domain.Bookings.BookingParty();
        if (r.Payer != null) draft.Payer = CreateBookingCommandHandler.MapParty(r.Payer);
        if (r.PickUpAddress != null) draft.PickUpAddress = CreateBookingCommandHandler.MapParty(r.PickUpAddress) ?? new HiavaNet.Domain.Bookings.BookingParty();
        if (r.DeliveryPoint != null) draft.DeliveryPoint = CreateBookingCommandHandler.MapParty(r.DeliveryPoint) ?? new HiavaNet.Domain.Bookings.BookingParty();
